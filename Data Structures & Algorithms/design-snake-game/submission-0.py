class SnakeGame:
    ...
    def move(self, direction: str) -> int:
        ...
        next_food_item = self.food[self.food_index] if \
        self.food_index < len(self.food) else None

        if self.food_index < len(self.food) and next_food_item[0] == newHead[0] \
        and next_food_item[1] == newHead[1]:
            self.food_index += 1
        else:
            tail = self.snake.pop()
