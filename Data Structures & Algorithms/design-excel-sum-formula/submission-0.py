class Excel:
    ...
    def sum(self, row: int, column: str, numbers: List[str]) -> int:
        ...
        return summ
    def topSort(self, r: int, c: int):
        for i in range(len(self.formulas)):
            for j in range(len(self.formulas[0])):
                if self.formulas[i][j] is not None and \
                (chr(ord('A') + c) + str(r + 1)) in self.formulas[i][j][0]:
                    self.topSort(i,j)
    ...
    def execute_stack(self):
        while self.stack:
            ...
            if len(self.formulas[top[0]][top[1]][0]) > 0:
                self.calculate_sum(top[0], top[1],
                self.formulas[top[0]][top[1]][0])
    ...
    def calculate_sum(self, r: int, c: int, cells: dict) -> int:
        ...
        for s, count in cells.items():
            x = int(s[1:]) - 1
            y = ord(s[0]) - ord('A')
            total += (self.formulas[x][y][1] if
            self.formulas[x][y] is not None else 0)* count
